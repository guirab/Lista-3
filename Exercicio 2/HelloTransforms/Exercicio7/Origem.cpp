...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float cores[] = {1, 0.7, 1, 0.7, 0.7, 1, 0.8, 1, 0.7, 0};

		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 6; j++) {
				model = glm::mat4(1); //matriz identidade

				model = glm::translate(model, glm::vec3(100 + i * 50, 100 + j * 50, 0.0));
				...
				float r = cores[(i + j) % 10];
				float g = cores[(i + j + 3) % 10];
				float b = cores[(i + j + 6) % 10];
